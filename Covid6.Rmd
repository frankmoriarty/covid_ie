...
author: "Ciaran Prendergast and Frank Moriarty"
date: "4/27/2020" #Does this need to be updated?
output: 
  html_document:
    code_folding: hide
    number_sections: yes
...
#possible to add legend for total cases line too?
...
plot2 <- plot + labs(title = "Evolution of the Mode of Transmission")+
 scale_y_continuous("% of New Infections", labels = percent_format(), limits=c(0,1))+ #I actually think this is % of all cases
  scale_x_date("Date")
...
As we can see in the map, **Dublin** and **Cavan** have suffered the highest relative proportion of cases, followed by Monaghan, Westmeath, Kildare and Wicklow. As of `r the.date` , it is mainly counties of the west that have been least affected. #I wonder is there a way to insert code instead of Dublin/Cavan etc that resolves to the county name for the county with the highest ranks? The multiple Dublin local authority areas may make this too difficult - perhaps we just have to keep an eye on this and update if this changes?
...
## ICU Admissions #Would it be possible to move hospitalisations above ICU admissions? May be a little more logical assuming one may transition from diagnosed to hospitalised to ICU admission.

The **Intensive Care Units** in Ireland saw peak patient influx between late March and early April, and the percentage increase in ICU admissions began to below 0 on occasion from late April. #I actually think these <0 values are corrections in reporting i.e. total number of cases admitted to ICU thus far are reported daily, we infer new admissions from this. 
<br><br><br><br>
```{r, include=TRUE, message=FALSE, warning=FALSE}
library(tidyverse)
library(RCurl)
...
Hospital admissions have fallen slowly after early peaks near April 1st. The percentage increase on total hospital admissions trended downwards throughout the crisis. #I think for both this and the ICU admissions it might be daily admissions plotted rather than cumulative total like the new cases graph? Also would it be possible to add the blue line legend too?
...
&nbsp;
<hr />
<p style="text-align: center;">A work by <a href=""> Ciaran Prendergast and Frank Moriarty</a></p>
...
<p style="text-align: center;">
    ...
    <a href="https://www.linkedin.com/in/yan-holtz-2477534a/" class="fa fa-linkedin"></a> #Can we take these out?
